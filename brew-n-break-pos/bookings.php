...
try {
    ...
    if (!$conn->connect_error) {
        $res = $conn->query("SELECT *, DATEDIFF(check_out, check_in) * 3500 AS amount FROM bookings WHERE room='Airbnb' ORDER BY check_in DESC");
...
<title>Booking Management – Brew n' Break</title>
...
.tab-btn{padding:7px 22px;border:none;background:transparent;border-radius:6px;font-size:13px;font-family:'Lato',sans-serif;color:var(--text-mid);cursor:pointer;font-weight:400;transition:background .2s,color .2s;text-decoration:none;display:inline-block;}
...
#deleteModal{display:none;position:fixed;inset:0;z-index:99998;align-items:center;justify-content:center;background:rgba(0,0,0,0.55);backdrop-filter:blur(3px);}
...
.del-sub{font-size:13px;color:var(--muted);margin-bottom:24px;line-height:1.5;}
.del-btns{display:flex;gap:10px;justify-content:center;}
.del-cancel{flex:1;padding:10px 0;border-radius:10px;border:1.5px solid rgba(0,0,0,0.15);background:transparent;color:var(--text-mid);font-size:13px;font-weight:600;cursor:pointer;transition:background .2s;}
.del-cancel:hover{background:rgba(0,0,0,0.06);}
.del-confirm{flex:1;padding:10px 0;border-radius:10px;border:none;background:#c0392b;color:#fff;font-size:13px;font-weight:700;cursor:pointer;transition:background .2s;}
...
<div class="layout">
  <aside class="sidebar">
    ...
    <a class="nav-item active" href="bookings.php"><span>📅</span><span class="tip">Bookings</span></a>
    ...
    <div class="card">
      <div class="toolbar">
        ...
        <div class="toolbar-right">
          ...
          <button class="icon-btn" title="Add Booking" onclick="openModal()">➕</button>
        </div>
      </div>
        ...
        <table>
          ...
          <tbody id="tableBody">
            <?php foreach ($bookings as $b):
              ...
              $ci     = date('F j, Y', strtotime($b['check_in']));
              $co     = date('F j, Y', strtotime($b['check_out']));
              $nights = max(1, (int)$b['amount'] / 3500);
              $amt    = '₱'.number_format($b['amount'], 2);
            ?>
            <tr data-search="<?= strtolower(htmlspecialchars($b['booking_code'].' '.$b['guest_name'])) ?>">
              ...
              <td><span class="<?= $sc ?>"><?= htmlspecialchars($b['status']) ?></span></td>
              <td>
                <div class="action-wrap" id="wrap-<?= $b['id'] ?>">
                  <button class="action-btn" onclick="toggleDD(<?= $b['id'] ?>, this)">•••</button>
                  <div class="dropdown" id="dd-<?= $b['id'] ?>">
                    <a href="#" onclick="editBooking(<?= $b['id'] ?>,'<?= htmlspecialchars($b['status']) ?>');return false;">✏️ Edit Status</a>
                    <a href="#" class="danger" onclick="deleteBooking(<?= $b['id'] ?>);return false;">🗑️ Delete</a>
                  </div>
                </div>
              </td>
            </tr>
  ...
  <div class="modal">
    ...
    <div class="form-row">
      <div class="form-group">
        <label>Booking ID <span style="font-size:10px;color:var(--gold);letter-spacing:.5px;">AUTO</span></label>
        <input type="text" id="fCode" readonly style="background:rgba(0,0,0,0.06);cursor:default;color:var(--muted);"/>
      </div>
      <div class="form-group">
        <label>Guest Name</label>
        <input type="text" id="fGuest" placeholder="e.g. Juan Dela Cruz"/>
      </div>
    </div>
    ...
    <div class="modal-actions">
      ...
      <button class="btn btn-primary" onclick="submitBooking()">Save</button>
    </div>
  </div>
</div>
...
function renderPage(){
  ...
  if(total === 0){ info.textContent = 'No bookings found'; }
  else { info.textContent = 'Showing '+(start+1)+'–'+Math.min(end, total)+' of '+total+' bookings'; }
}
...
async function openModal(){
  ...
  try {
    const r = await fetch('booking_action.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'next_code'})});
    const d = await r.json();
    if(d.success) document.getElementById('fCode').value=d.code;
  } catch(e){}
}
...
async function submitBooking(){
  ...
  if(!code||code==='…'||!guest||!checkIn||!checkOut){ errEl.textContent='Please fill in all required fields.'; return; }
  if(checkOut<=checkIn){ errEl.textContent='Check-out must be after check-in.'; return; }

  const res  = await fetch('booking_action.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'add',code,guest,room:'Airbnb',status,checkIn,checkOut})});
...
async function submitEditStatus(){
  ...
  const res    = await fetch('booking_action.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'edit_status',id,status})});
  const data   = await res.json();
...
async function confirmDeleteBooking(){
  if(!_deleteId) return;
  const res  = await fetch('booking_action.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete',id:_deleteId})});
  const data = await res.json();
  hideDeleteModal();
...
<div id="deleteModal" onclick="if(event.target===this)hideDeleteModal()">
  <div class="del-box">
    <div class="del-icon">🗑️</div>
    <div class="del-title">Delete Booking?</div>
    <div class="del-sub">This action cannot be undone. The booking will be permanently removed.</div>
    <div class="del-btns">
      <button class="del-cancel" onclick="hideDeleteModal()">Cancel</button>
      <button class="del-confirm" onclick="confirmDeleteBooking()">Delete</button>
    </div>
  </div>
</div>
<div id="bellPopup">
