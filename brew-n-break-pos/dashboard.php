...
if (strtolower($_SESSION['role'] ?? '') === 'staff') { header('Location: /brew-n-break-pos/staff.php'); exit; }
...
try {
    ...
    if (!$conn->connect_error) {
        ...
        $r = $conn->query("SELECT COALESCE(SUM(DATEDIFF(check_out,check_in)*3500),0) FROM bookings WHERE DATE(created_at)=CURDATE()"); if ($r) $bookingRev = (float)$r->fetch_row()[0];
        $billiardSchedule = [];
        $r = $conn->query("
...
<div class="layout">
  <aside class="sidebar">
    <a class="nav-item active" href="dashboard.php"><span>🏠</span><span class="tip">Dashboard</span></a>
  ...
  <main class="main">
    ...
    <script type="text/babel">
      ...
      function StatCards() {
        ...
        useEffect(() => {
          function fetchStats() {
            fetch('http://localhost:5000/api/dashboard-stats')
              .then(res => res.json())
              .then(data => {
                if (data.success) setStats({ orders: data.orders, hours: data.hours, bookings: data.bookings, revenue: data.revenue });
              })
              .catch(() => {});
          }
          fetchStats();
        ...
        const cards = [
          { label: "Today's Orders",       value: stats.orders.toLocaleString() },
          { label: "Today's Table Hours",  value: parseFloat(stats.hours).toFixed(1) },
          { label: "Today's Bookings",     value: stats.bookings.toLocaleString() },
          { label: "Today's Revenue",      value: '₱' + parseFloat(stats.revenue).toLocaleString('en-PH', {minimumFractionDigits:2}) },
        ];
      ...
      ReactDOM.createRoot(document.getElementById('stat-cards-root')).render(<StatCards />);
    </script>
  ...
  async function pollBell(){
    try {
      ...
      if (undismissed.length > 0) {
        ...
        const fresh = undismissed.filter(a => !dismissed.has(canonId(a.id)));
        if (fresh.length) {
